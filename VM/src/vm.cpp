...
void VM::execute() {
  for(;;) {
    ...
    case OpCode::ADD_CONST: {
      //std::cout << "Add Const x" << i.parameters.add.target << " := x" << i.parameters.add.source << " + " << i.parameters.add.constant << std::endl;
	...
	this->data.push_back(0);
      ...
      this->stack.push_back(Activation(this, next_offset, next_len, ret_targ, -1, ind));
      this->instruction_pointer++;
      break;
    }
    case OpCode::ARG: {

...
namespace Theo {
  ...
  private:
	...
	ERR
      ...
      // reduce action
      int left; // left side of the rule that is reduced, used as index into jump table
  ...
  LRParser<SemanticType, TokenType>::generateParseTables() {
    ...
    action = std::vector<std::vector<Action>>(tables_height, std::vector<Action>(action_width, {.t = Action::ERR}));
    jump = std::vector<std::vector<int>>(tables_height, std::vector<int>(jump_width, -1));

    auto place_shift = [&](int state, int terminal, int target) -> void {
      switch(action[state][terminal].t) {
      case Action::REDUCE: {
	res.push_back({
	    GenerationResult::SHIFT_REDUCE_ERR,
	    "(ps) shift-reduce error in state " + std::to_string(state) + " on terminal " + std::to_string(terminal)
	  });
	break;
      }
      default:
    ...
    auto place_reduce = [&](int state, int terminal, const LRElement &left, int size) -> void {
      switch(action[state][terminal].t) {
      case Action::REDUCE: {
	res.push_back({
	    GenerationResult::REDUCE_REDUCE_ERR,
	    "(pr) reduce-reduce error in state " + std::to_string(state) + " on terminal " + std::to_string(terminal)
	  });
	break;
      }
      case Action::SHIFT: {
	res.push_back({
	    GenerationResult::SHIFT_REDUCE_ERR,
	    "(pr) shift-reduce error in state " + std::to_string(state) + " on terminal " + std::to_string(terminal)
	  });
	break;
      }
      default:
	auto a = G.actions[left.left][left.alternative];
	action[state][terminal] = {.t = Action::REDUCE, .left = (int)left.left.index, .beta = size, .action = a};
      }
    };
	...
	res.push_back({
	    GenerationResult::REDUCE_REDUCE_ERR,
	    "accept-reduce error in state " + std::to_string(state) + " on terminal " + std::to_string(terminal)
	  });
	break;
      }
      case Action::SHIFT: {
	res.push_back({
	    GenerationResult::SHIFT_REDUCE_ERR,
	    "accept-shift error in state " + std::to_string(state) + " on terminal " + std::to_string(terminal)
	  });
	break;
      }
      default:

...
";
  ...
  if (mer.macros.size() != 3){
    std::cerr << "expected to find 3 macros, found " << mer.macros.size() << std::endl;
    error = true;
  } else {
	...
	|| mer.macros[0].template_token_indices[1] != 2) {
      std::cerr << "first macro of unexpected format:" << std::endl
		...
		<< 1 << " == " << mer.macros[0].content_constraint_token_indices[0] << std::endl
		<< 0 << " == " << mer.macros[0].template_token_indices[0] << std::endl
		<< 2 << " == " << mer.macros[0].template_token_indices[1] << std::endl;
      error = true;
    }

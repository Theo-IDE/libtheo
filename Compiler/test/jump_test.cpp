...
int main() {
  ...
  Grammar::Symbol
    S_prime = sg.createNonTerminal(),
    S = sg.createNonTerminal(),
    C = sg.createNonTerminal(),
    DUMMY = sg.createNonTerminal(),
  ...
  sg.add(C >> (c, C), _);
  sg.add(C >> d, _);
  sg.add(DUMMY >> dollar, _); // so .calculateFirstSets() will calculate a first set for $
